...
class WebsiteController extends Controller
{
    ...
    return view('website/discover',compact('Industry','Expertise','Business', 'coaches'));
    ...
    $coaches = User::where('role', 'coach')->where('status', 2)->where('name', 'LIKE', "%{$search}%")->get();    
   
    return view('website/search_discover',compact('coaches'));
    ...
    public function contactInquiry(Request $request)
    {
      ...
      $email_data = array(
        ...
        Mail::send('email.contactMail', $email_data, function ($message) use ($email_data) {
            $message->to($email_data['emails'], $email_data['name'])
